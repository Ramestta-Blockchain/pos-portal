...
module.exports = async (deployer) => {
  ...
  const RootChainManagerInstance = await RootChainManager.at(
    contractAddresses.root.RootChainManagerProxy
  );

  const ERC20PredicateInstance = await ERC20Predicate.at(
    contractAddresses.root.ERC20PredicateProxy
  );
  const MintableERC20PredicateInstance = await MintableERC20Predicate.at(
    contractAddresses.root.MintableERC20PredicateProxy
  );

  const ERC721PredicateInstance = await ERC721Predicate.at(
    contractAddresses.root.ERC721PredicateProxy
  );
  ...
  const ERC1155PredicateInstance = await ERC1155Predicate.at(
    contractAddresses.root.ERC1155PredicateProxy
  );
  ...
  const EtherPredicateInstance = await EtherPredicate.at(
    contractAddresses.root.EtherPredicateProxy
  );

  const DummyMintableERC20Instance = await DummyMintableERC20.at(
    contractAddresses.root.DummyMintableERC20
  );
  const DummyMintableERC721Instance = await DummyMintableERC721.at(
    contractAddresses.root.DummyMintableERC721
  );
  const DummyMintableERC1155Instance = await DummyMintableERC1155.at(
    contractAddresses.root.DummyMintableERC1155
  );

  console.log("Setting StateSender");
  await RootChainManagerInstance.setStateSender(
    contractAddresses.root.DummyStateSender
  );

  console.log("Setting ChildChainManager");
  await RootChainManagerInstance.setChildChainManagerAddress(
    contractAddresses.child.ChildChainManagerProxy
  );
  ...
  await DummyMintableERC20Instance.grantRole(
    PREDICATE_ROLE,
    MintableERC20PredicateInstance.address
  );

  console.log("Granting predicate role on DummyMintableERC721");
  await DummyMintableERC721Instance.grantRole(
    PREDICATE_ROLE,
    MintableERC721PredicateInstance.address
  );

  console.log("Granting predicate role on DummyMintableERC1155");
  await DummyMintableERC1155Instance.grantRole(
    PREDICATE_ROLE,
    MintableERC1155PredicateInstance.address
  );

  console.log("Registering ERC20Predicate");
  const ERC20Type = await ERC20PredicateInstance.TOKEN_TYPE();
  await RootChainManagerInstance.registerPredicate(
    ERC20Type,
    ERC20PredicateInstance.address
  );
  ...
  await RootChainManagerInstance.registerPredicate(
    ERC721Type,
    ERC721PredicateInstance.address
  );
  ...
  await RootChainManagerInstance.registerPredicate(
    ERC1155Type,
    ERC1155PredicateInstance.address
  );

  console.log("Registering MintableERC1155Predicate");
  const MintableERC1155Type =
    await MintableERC1155PredicateInstance.TOKEN_TYPE();
  await RootChainManagerInstance.registerPredicate(
    MintableERC1155Type,
    MintableERC1155PredicateInstance.address
  );
  ...
  await RootChainManagerInstance.registerPredicate(
    EtherType,
    EtherPredicateInstance.address
  );
  ...
  await RootChainManagerInstance.mapToken(
    ...
    contractAddresses.child.RamaWMATIC,
    EtherType
  );
};
